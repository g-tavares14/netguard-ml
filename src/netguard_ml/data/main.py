...
Uso (na raiz do repo, com o dataset já baixado):
    ...
    .venv/bin/python src/netguard_ml/data/main.py
    .venv/bin/python src/netguard_ml/data/main.py --on recorte
...
def icmp(df: pd.DataFrame) -> dict:
    nomes_wishlist = [
        c
        for c in df.columns
        if any(tok in c.lower() for tok in ("echo", "ping", "icmp_type", "icmp_code", "type", "code"))
    ...
    ping = df[df["Label"] == "Recon-PingSweep"] if "Label" in df.columns else df.iloc[0:0]
    return {
        ...
        "Recon-PingSweep_ICMP_gt_0": int((ping["ICMP"] > 0).sum()) if tem_icmp and len(ping) else 0,
...
def timestamps(df: pd.DataFrame) -> dict:
    suspeitas = [
        c
        for c in df.columns
        if any(tok in c.lower() for tok in ("time", "timestamp", "date", "epoch", "clock"))
    ]
    iat = {}
...
def main() -> None:
    ...
    if args.on != "train":
        print(
            "aviso: regras do projeto devem ser confirmadas com --on train"
        )
